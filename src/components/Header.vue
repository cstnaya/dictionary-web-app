<template>
  <header class="flex items-center justify-between pt-6 pb-3">
    <h1>
      <router-link to="/" class="text-4xl text-slate-500 dark:text-neutral-500 md:text-5xl"
        ><font-awesome-icon icon="fas fa-glasses"
      /></router-link>
    </h1>
    <ul class="flex gap-5">
      <li class="flex items-center">
        <select
          class="px-2 text-right dark:bg-neutral-800 dark:text-white"
          :value="fontSize"
      ...
      <li class="flex items-center border-l-2 border-l-slate-200 pl-4">
        ...
        <font-awesome-icon
          icon="fas fa-moon"
          v-if="darkMode"
          class="w-8 text-xl dark:text-violet-400"
        />
